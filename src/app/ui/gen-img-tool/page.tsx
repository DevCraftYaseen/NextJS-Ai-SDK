...
import { ChatMessage } from "@/app/api/gen-img-tool/route";
...
export default function GenImgTool() {
        ...
        <div className="w-full max-w-2xl animate-fade-in-up flex flex-col items-center gap-8">
          <div className="text-center space-y-3">
            ...
            <p className="text-text-tertiary text-sm max-w-sm mx-auto">
              Describe any image you want and AI will create it for you using advanced diffusion models.
            </p>
          </div>
          ...
          {generationTimeout && (
            <div className="animate-fade-in flex items-center gap-2.5 rounded-lg border border-amber-500/20 bg-amber-50 p-3.5 text-amber-700 text-sm">
              ...
              <div className="flex-1">
                ...
                <p className="text-xs opacity-80">The image service may be experiencing issues. You can stop and try again with a simpler prompt.</p>
              </div>
              <button
                onClick={stop}
                      ...
                      case "tool-generateImage": {
                        switch (part.state) {
                          ...
                          case "output-available": {
                            // The output is a base64 string from the tool
                            const base64Data = part.output as string;
                            return (
                              <div key={`${message.id}-tool-${index}`} className="mt-3 space-y-3">
                                <ImageCompleteState />
                                <ImageDisplay 
                          ...
                          case "output-error":
                            return (
                              <div key={`${message.id}-tool-${index}`} className="mt-3">
                                <ImageErrorState error={part.errorText || undefined} />
                              </div>
                            );
                          default:
                            return null;
