...
export default function MCPToosPage() {
        ...
        <div className="w-full max-w-2xl animate-fade-in-up flex flex-col items-center gap-8">
          ...
          <div className="text-center space-y-3">
            ...
            <p className="text-text-tertiary text-sm max-w-sm mx-auto">
              Ask about stock prices and more using Model Context Protocol (MCP) tools.
            </p>
          </div>
          ...
          <p className="text-xs text-text-tertiary">
            Powered by Gemini &middot; MCP Tools via Vercel AI SDK
          </p>
        </div>
      </div>
    );
                  ...
                  {message.parts.map((part, index) => {
                    switch (part.type) {
                      ...
                      case 'dynamic-tool': {
                        ...
                        const isStockTool = toolName.toLowerCase().includes('stock');
                        ...
                        switch (part.state) {
                          case 'input-streaming':
                            return (
                              <div
                                ...
                                className="mt-2 rounded-lg border border-border bg-background p-3 animate-fade-in"
                              >
                                <div className="flex items-center gap-2 mb-2">
                                  <div className="h-4 w-4 rounded-full bg-accent/20 flex items-center justify-center">
                                    <div className="h-1.5 w-1.5 rounded-full bg-accent animate-pulse" />
                                  </div>
                                  <span className="text-xs font-medium text-text-secondary uppercase tracking-wide">{toolName}</span>
                                </div>
                                <div className="text-sm text-text-tertiary">Receiving request...</div>
                              </div>
                            );
                          case 'input-available':
                            return (
                              <div
                                key={`${message.id}-${toolName}-${index}`}
                                className="mt-2 rounded-lg border border-accent/30 bg-accent/5 p-3 animate-fade-in"
                              >
                                <div className="flex items-center gap-2 mb-2">
                                  <div className="h-5 w-5 rounded-lg bg-accent-light flex items-center justify-center">
                                ...
                                <div className="flex items-center gap-2 text-sm text-text-primary">
                                  <span className="text-text-tertiary">Processing request</span>
                                  <span className="text-text-tertiary">...</span>
                                </div>
                              </div>
                            );
                          case 'output-available':
                            // Format stock price output
                            const output = part.output as { symbol?: string; price?: number; currency?: string; change?: number; changePercent?: number } | string;
                            let stockData: { symbol?: string; price?: number; currency?: string; change?: number; changePercent?: number } = {};
                            
                            if (typeof output === 'string') {
                              try {
                                stockData = JSON.parse(output);
                              } catch {
                                stockData = { symbol: 'Unknown', price: 0 };
                              }
                            } else {
                              stockData = output;
                            }
                            ...
                            return (
                              <div
                                key={`${message.id}-${toolName}-${index}`}
                                className="mt-2 rounded-lg border border-border bg-surface-raised p-4 animate-fade-in"
                              >
                                <div className="flex items-center gap-2 mb-3">
                                  ...
                                  <span className="text-xs font-semibold text-text-primary uppercase tracking-wide">{isStockTool ? 'Stock Price' : toolName} Result</span>
                                </div>
                                
                                {isStockTool && stockData.symbol ? (
                                  <div className="bg-background rounded-lg p-4 border border-border-light">
                                    <div className="flex items-center justify-between mb-2">
                                      <span className="text-lg font-bold text-text-primary">{stockData.symbol}</span>
                                      <span className={`text-sm font-medium px-2 py-1 rounded ${isPositive ? 'bg-emerald-100 text-emerald-700' : 'bg-red-100 text-red-700'}`}>
                                        {isPositive ? '+' : ''}{stockData.changePercent?.toFixed(2) || '0.00'}%
                                      </span>
                                    </div>
                                    <div className="flex items-baseline gap-1">
                                      <span className="text-3xl font-bold text-text-primary">
                                        {stockData.currency || '$'}{stockData.price?.toFixed(2) || '0.00'}
                                      </span>
                                  ...
                                  <div className="text-[15px] leading-7 text-text-primary bg-background rounded-lg p-3 border border-border-light">
                          ...
                          case 'output-error':
                            return (
                              <div
                                key={`${message.id}-${toolName}-${index}`}
                                className="mt-2 rounded-lg border border-error/30 bg-error-light/50 p-3 animate-fade-in"
                              >
                                <div className="flex items-center gap-2 mb-2">
                                  ...
                                  <span className="text-xs font-medium text-error uppercase tracking-wide">{toolName} Error</span>
                                </div>
                                <p className="text-sm text-error/80">{part.errorText || 'An error occurred while processing the request.'}</p>
                              </div>
                            );
                          default:
                            return null;
